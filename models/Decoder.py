...
from tool.loss_utils import farthest_point_sample
...
class LocalRefine(nn.Module):
    ...
    def forward(self, pc):
        ...
        mu    = pc.mean(dim=1, keepdim=True)     # [B,1,3]
        sigma = pc.std(dim=1,  keepdim=True)     # [B,1,3]

        normed = (pc - mu) / (sigma + 1e-6)      # [B,N,3]


        B, N, _ = normed.shape
        normed_flat  = normed.view(B*N, 3)
        offset_flat  = self.offset_mlp(normed_flat)  # [B*N, 3]
        offset       = offset_flat.view(B, N, 3)
...
class Decoder_Network(nn.Module):
    ...
    def forward(self, x, x_part):
        ...
        x_fine = self.local_refine(x_1)
        complete = torch.cat((x_fine, x_partial), dim=1)
